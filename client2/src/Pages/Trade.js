...
const Trade = () => {
  ...
  useEffect(() => {
    ...
    const divElement = document.getElementById('chartBox');
    if (divElement) {
      const newWidth = divElement.offsetHeight; // Set the desired offset width here
      // divElement.style.width = `${newWidth}px`;
      setchartHeight(newWidth);
      console.log("This is Height of DIV " + chartHeight);
  ...
  const reducer = (state, action) => {
    switch (action.type) {
      case 'update':
        localStorage.setItem('initstate', JSON.stringify(action.payload));
        console.log("Hii iamhere ", typeof action.payload);
  ...
  console.log("type is ", typeof initstate);
  if (!initstate)
    initstate = {
      symbol: 'Select coin ',
      custom: false,
          ...
          >
            ...
            >
              ...
              >
                ...
                <div className="main-chart mb15" id='chartBox'>
                  <AdvancedChart
                    widgetProps={{
                ...
                <Box id="market"
                  sx={{
                    ...
                    gap: { xs: '0px', sm: '0px' }, width: { xs: window.innerWidth, sm: 'auto' },
                    backgroundColor: (state2.color == '#ffffff' ? '#f5f5f5' : '#1b202e')


                    // flexDirection: '@media only screen and (max-width: 768px) ? "column" : "row"',
                  }}
                >
                  {/* <div style={{ paddingLeft: '30px', backgroundColor: '', display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <Typography>
